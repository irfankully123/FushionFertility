...
class PutDoctorRequest extends FormRequest
{
    /**
    ...
    {
        $doctor = Route::current()->parameter('doctor');
        ...
        return [
            ...
            'userData.email' => 'required|email|unique:users,email,' . $doctor->user()->firstOr()->id,
            'userData.consultant_meeting_time'=>['required', 'string', Rule::notIn(['0']), Rule::in(['15', '20','25','30','35','40','45','60'])],
            'userData.password' => 'nullable|min:8|max:20|regex:/^(?=.*?[A-Z])(?=.*?[a-z])(?=.*?[0-9])(?=.*?[#?!@$%^&*-]).{6,}$/',
            'userData.confirm' => 'nullable|min:8|max:20|required_with:userData.password|same:userData.password',
