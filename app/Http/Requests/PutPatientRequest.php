...
class PutPatientRequest extends FormRequest
{
    ...
    {
        $patient = Route::current()->parameter('patient');
        return [
            'name' => 'required|min:3',
            'email' => 'required|unique:users,email,' . ($patient->user() ? $patient->user()->first()->id : 'NULL'),
            'password' => 'nullable|min:8|max:20|regex:/^(?=.*?[A-Z])(?=.*?[a-z])(?=.*?[0-9])(?=.*?[#?!@$%^&*-]).{6,}$/',
            'confirm' => 'nullable|min:8|max:20|required_with:password|same:password',
            ...
            'zip_code' => 'required|regex:/^\d{5}(?:[-\s]\d{4})?$/ | min:5',
