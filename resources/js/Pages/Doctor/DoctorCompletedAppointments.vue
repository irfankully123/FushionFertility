...
watch(search, debounce((value) => {
    router.get(route('doctor_dashboard.appointmentsHistory', id), {search: value}, {
        preserveState: true,
        replace: true
    });
}, 500));
...
const handleSubmit = () => {
    form.post(route('doctor_dashboard.appointmentsHistoryRegen', appointment.id), {
        preserveScroll: true,
        onSuccess: () => {
            form.reset();
            form.clearErrors();
...
const downloadPDF = (url) => {
    if (url != null) {
        const fullUrl = `/storage/doctor/${url}`
...
<template>
    ...
    <Modal v-if="state.showModal" title="Regenerate Prescription">
        <template #header>
        ...
        <template #footer>
            ...
            <SpinnerButton @click="handleSubmit" type="button" class="btn btn-danger" :processing="form.processing">
                Regenerate PDF
            </SpinnerButton>
        </template>
    </Modal>
                ...
                <table class="table table-hover">
                    ...
                    <tr v-for="(appointment,index) in props.appointments.data" :key="index">
                        ...
                        <td>
                            <div class="row">
                                <div class="col-6 ">
                                    <button @click="downloadPDF(appointment.doctor_prescription.pdf_url)" type="button"
                                            class="btn btn-dark mx-1">Download PDF
                                    </button>
                                    <button @click="showPrescriptionModal(appointment.id)" type="button"
                                            class="btn btn-warning mx-1">Regenerate PDF
                                    </button>
                                </div>
                            </div>
                        </td>
